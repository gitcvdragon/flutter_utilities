...
/*
class SerialFetchObservable<T> extends GetxController {
...
/*class UnprotectedSerialFetchObservable<T>
    extends SerialFetchObservable<T> {
  /// Used for listening to data of which can be fetched by parts.
  /// The parts are determined by `limit` and `offset`.
  ///
  /// NOTE: UnprotectedSerialFetchManager grants modification access to the data
  ///
  /// REQUIRED PARAMETERS: Function `urlGenerator`, Function `responseBodyToData`, Function `add`
  ///
  /// * Uri Function(int `offset`, int `limit`) urlGenerator: The function that takes the offset and the limit and returns the Uri for fetching data
  ...
  ///
  /// Please note that the data will be converted to Rx type
  UnprotectedSerialFetchObservable({
    required Uri Function(int offset, int limit) urlGenerator,
    required T Function(String responseBody) responseBodyToData,
    required T Function(T? oldData, T newData) add,
    required bool Function(T data) isEmpty,
    void Function()? postInit,
    String name = '',
  }) : super(
          urlGenerator: urlGenerator,
          responseBodyToData: responseBodyToData,
          add: (x, y) => add(x, y),
          isEmpty: (x) => isEmpty(x),
          name: name,
          postInit: postInit,
  ...
  void _printExceptRelease(String message) {
    printExceptRelease(
        'UnprotectedSerialFetchManager: ${_getPrefix()}$message');
  }
}*/

class SerialFetchObservable<T> extends PartDataFetchObservable<T> {
  /// The url to fetch the data
  final Uri Function(T? data, int offset, int limit) urlGenerator;
...
class SerialFetchObservableX<T, X> extends PartDataFetchObservableX<T, X> {
  /// The url to fetch the data
  final Uri Function(T? data, X? otherData, int offset, int limit) urlGenerator;

  SerialFetchObservableX({
    required this.urlGenerator,
    required super.responseBodyToData,
    required super.add,
    required super.isEmpty,
...
//Helper functions
List<T> listAddition<T>(List<T>? oldData, List<T>? newData) =>
    (oldData == null || newData == null)
        ? (oldData ?? newData ?? const [])
        : (List<T>.from(oldData)..addAll(newData));

bool listIsEmpty<T>(List<T>? data) => data == null || data.isEmpty;
